...
if [ "$mlsuccess" == "0" ] ; then
  matlab -singleCompThread -nodisplay -nodesktop -nosplash -nojvm -r "${n}run('calcpi_array.m');quit;"
elif [ "$ocsuccess" == "0" ] ; then
  octave --no-gui --silent --eval "maxNumCompThreads=1;${n}run('calcpi_array.m');"
else
